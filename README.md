# *SoftSpiders* для разработчиков на *Jepria*

На всякий случай: на данный момент *SoftSpider* (кратко - *SS*) название рабочее.
...
Цель - максимально облегчить жизнь прикладным разработчикам. 
...
#### Стартеры, шаблоны, архетипы, ...

Серьёзные поиски сегодня можно вести только на GitHub.
Если что-то находится через Google или Youtube, будь то статьи или видео, то в 99% случаев ссылки оттуда всё равно ведут
на GitHub. 
...
![Spring Boot - стартеры](images/starters-spring-boot.png)

Spring Boot - Maven-архетипы:
![Spring Boot - Maven-архетипы](images/maven-archetype-spring-boot.png)

Для React:
![PL/SQL - стартеры](images/maven-archetype-react.png)


### Зоопарк архитектур или Золотой молоток ?

Почему не декомпозируем Jepria ?
...
Acyclic Graph*). В дальнейшем для краткости иерархии таково вида будем называть DAG-иерархиями.

Такими множествами, которые в данном случае частично упорядочиваются являются множества тегов.

### Иерархия как индекс

Если присмотреться, то получившаяся иерархия представляет собой готовый индекс для поиска по тегам.

Особенность в том, что обычно индексы строятся по набору данных. 

### Иерархия как средство навигации

TBD


### Что в принципе представимо в виде DAG-иерархий ?

Многое. Приведём только пару примеров
  
#### Раскладки в пользовательских интерфейсах: 

![ui-layout-иерархия](images/layout-main-header-footer-full-hierarchy.png ) 

Возможно, реализации полнофункциональных иерархий потребуют дополнительных соглашений об именовании тегов (например,
использования '.' в тегах для описания древовидных структур). 

#### REST API 

Нужны картинки с CRUD
